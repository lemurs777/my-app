...
function TableLeagueCustom({ data }: { data: TableLeagueCustomData }) {
		...
		<>
						...
						<div key={index} className={styles.row}>
							<div
								className={clsx(
									...
									{
										[styles.cellPlayoff]:
											item.playTournamentClass === 'playoff',
									},
									{
										[styles.cellQualification]:
											item.playTournamentClass === 'qualification',
									},
			...
			{data.btnShowTable ? (
				<div className={styles.btnWrapper}>
					...
					>
						{showTable ? 'Скрыть таблицу' : 'Показать всю таблицу'}
					</button>
				</div>
			) : null}
			{data.info ? (
				<div className={styles.info}>
					<p className={styles.infoText}>
						Если две (или более) команды имеют одинаковое количество очков,
						следующие факторы помогут выявить победителя:
					</p>
