...
function StatisticsSearch() {
  ...
  return (
    <div className={styles.statisticsSearch}>
      <StatisticsNavigation name="search"/>
      <div className={styles.wrapper}>
        <div className={styles.mainWrapper}>
          <label className={styles.searchLabel}>
            <input className={styles.search} type="text" placeholder="Найти команду.." value={name}
          ...
          <div className={styles.listWrapper}>
            <ul className={styles.list}>
              {arrLinks.filter(item => item.name.toLowerCase().includes(name.toLowerCase())).map((item, index) => (
                (name === '') ? (
